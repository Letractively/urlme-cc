...
namespace ianhd.core.Data.Extensions
{
    public static class DapperExtensions
    {
        ...
        {
            IEnumerable<T> result = Dapper.Query<T>(cnn, DynamicQuery.GetInsertQuery(tableName, param), param);
            return result.First();
        }

        public static void Update(this IDbConnection cnn, string tableName, dynamic param)
        {
            Dapper.Execute(cnn, DynamicQuery.GetUpdateQuery(tableName, param), param);
        }

        public static IEnumerable<TFirst> Map<TFirst, TSecond, TKey>
            (
            this Dapper.GridReader reader,
        ...
        public static bool UpdateExisting<T>(this IDbConnection cnn, dynamic id, object properties, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            return Instance.UpdateExisting<T>(cnn, id, properties, transaction, commandTimeout);
        }

        public static bool UpdateExistingDynamic(this IDbConnection cnn, Assembly assembly, string tableName, dynamic id, dynamic properties, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var classType = assembly.GetTypes().FirstOrDefault(t => t.Name == tableName);

            if (classType == null) return false;

            var genMethodDef = typeof (DapperImplementor).GetMethod("UpdateExisting");
            var genMethod = genMethodDef.MakeGenericMethod(classType);
            var result = (bool)genMethod.Invoke(Instance, new object[] {cnn, id, properties, transaction, commandTimeout});

            return result;
        }

        public static bool DeleteExisting(this IDbConnection cnn, Assembly assembly, string tableName, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var classType = assembly.GetTypes().FirstOrDefault(t => t.Name == tableName);

            if (classType == null) return false;
            
            var genMethodDef = typeof(DapperImplementor).GetMethod("DeleteExisting");
            var genMethod = genMethodDef.MakeGenericMethod(classType);
            var result = (bool)genMethod.Invoke(Instance, new object[] { cnn, id, transaction, commandTimeout });
