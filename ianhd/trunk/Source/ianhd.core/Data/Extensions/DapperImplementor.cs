...
using System.Text;
using ianhd.core.Data.Extensions.Mapper;
using ianhd.core.Data.Extensions.Sql;
...
namespace ianhd.core.Data.Extensions
{
    public interface IDapperImplementor
    {
        ...
        bool UpdateExisting<T>(IDbConnection connection, dynamic id, dynamic properties, IDbTransaction transaction, int? commandTimeout) where T : class;
        ...
        bool DeleteExisting<T>(IDbConnection connection, dynamic id, IDbTransaction transaction, int? commandTimeout) where T : class;
    ...
    internal class DapperImplementor : IDapperImplementor
    {
        ...
        public bool UpdateExisting<T>(IDbConnection connection, dynamic id, dynamic properties, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            var result = this.Get<T>(connection, id, transaction, commandTimeout);

            if (result == null) return false;

            if (properties as IDictionary<string, object> == null)
            {
                var anonymousProps = properties.GetType().GetProperties();

                foreach (var toProperty in anonymousProps)
                {
                    var fromProperty = result.GetType().GetProperty(toProperty.Name) as PropertyInfo;

                    if (fromProperty == null || !fromProperty.CanWrite) continue;

                    var fromValue = toProperty.GetValue(properties, null);

                    fromProperty.SetValue(result, fromValue, null);
                }
            }
            else
            {
                ...
                {
                    var fromProperty = result.GetType().GetProperty(toProperty.Key) as PropertyInfo;

                    if (fromProperty == null || !fromProperty.CanWrite || toProperty.Value == null) continue;

                    fromProperty.SetValue(result, toProperty.Value, null);
                }
            }

            return this.Update(connection, result, transaction, commandTimeout);
        }
        ...
        public bool DeleteExisting<T>(IDbConnection connection, dynamic id, IDbTransaction transaction, int? commandTimeout) where T : class
        {
            var entity = this.Get<T>(connection, id, transaction, commandTimeout);
        ...
        protected GridReaderResultReader GetMultipleByBatch(IDbConnection connection, GetMultiplePredicate predicate, IDbTransaction transaction, int? commandTimeout)
        {
            ...
            Dapper.GridReader grid = connection.QueryMultiple(sql.ToString(), dynamicParameters, transaction, commandTimeout, CommandType.Text);
        ...
        {
            IList<Dapper.GridReader> items = new List<Dapper.GridReader>();
            foreach (var item in predicate.Items)
            {
                Dictionary<string, object> parameters = new Dictionary<string, object>();
                IClassMapper classMap = SqlGenerator.Configuration.GetMap(item.Type);
                ...
                Dapper.GridReader queryResult = connection.QueryMultiple(sql, dynamicParameters, transaction, commandTimeout, CommandType.Text);
