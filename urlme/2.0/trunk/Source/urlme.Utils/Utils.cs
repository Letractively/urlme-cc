...
namespace urlme.Utils
{
    public static class Shared
    {
        public static string ResolveUrl(this HttpContextBase context, string path)
        {
            if (string.IsNullOrEmpty(path) || path.ToLower().StartsWith("http"))
                return path;

            if (path.StartsWith("~") && context != null)
            {
                string applicationPath = "";
                if (context.Request.ApplicationPath.Length > 1)
                    applicationPath = context.Request.ApplicationPath;
                path = string.Format("http{0}://{1}{2}{3}", (context.Request.IsSecureConnection) ? "s" : "", context.Request.Url.Host.ToLower(), applicationPath.ToLower(), path.Remove(0, 1));
            }
            return path;
        }
